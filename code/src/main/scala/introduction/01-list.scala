...
object StreamAsList {
  ...
  def list[A](values: List[A]): Stream[Pure, A] = Stream(values: _*)
  ...
  // by `value`.
  def add(stream: Stream[Pure, Int], value: Int): Stream[Pure, Int] = stream.map(_ + value)

  // This method return a stream that only contains the values of the input
  // stream that match the predicate.
  def only[A](
      stream: Stream[Pure, A],
      predicate: A => Boolean
  ): Stream[Pure, A] = stream.filter(predicate)

  // This method should sum all the values in the given `stream` and return a
  // Stream containing just a single value, the total.
  def sum(stream: Stream[Pure, Int]): Stream[Pure, Int] = stream.fold1(_ + _)
}
